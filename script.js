document.addEventListener('DOMContentLoaded', () => {
    // --- DOM Elements ---
    const constitutionContentEl = document.getElementById('constitutionContent');
    ...
    const clearChecklistBtn = document.getElementById('clearChecklistButton'); // Get the new button

    // --- State Variables ---
    let originalHtml = '';
    let isShowingSearchResults = false;
    const checklistStorageKey = 'cardanoConstitutionChecklistStatus';
...
The overall goals when managing the technical/security parameters are to:
...
appropriate.
    ...
    function sanitizeForId(text) { /* ... */
        ...
        return text
            ...
            .substring(0, 50); // Limit length for sanity
     }
    function loadChecklistStatus() { /* ... */
        try {
            ...
            console.error("Error loading checklist status:", e);
            return {};
        }
     }
    function saveChecklistStatus(statuses) { /* ... */
        try {
            localStorage.setItem(checklistStorageKey, JSON.stringify(statuses));
        } catch (e) {
            console.error("Error saving checklist status:", e);
        }
     }
    function updateSectionStatus(sectionId, newStatus) { /* ... */
        const statuses = loadChecklistStatus();
        if (newStatus) {
            statuses[sectionId] = newStatus;
        } else {
            delete statuses[sectionId]; // Clear status if newStatus is null
        }
        saveChecklistStatus(statuses);
     }
    function createChecklistControls(sectionId) { /* ... */
        return `
            <div class="checklist-controls" data-section-id="${sectionId}">
                <button class="checklist-btn compliant" data-status="compliant" title="Mark as Compliant">✓</button>
                <button class="checklist-btn non-compliant" data-status="non-compliant" title="Mark as Non-Compliant">✕</button>
                <button class="checklist-btn review" data-status="review" title="Mark for Review">?</button>
            </div>
        `;
     }
    function applyChecklistVisualStatus(sectionId, status) { /* ... */
        const controlsContainer = constitutionContentEl.querySelector(`.checklist-controls[data-section-id="${sectionId}"]`);
         if (!controlsContainer) return;

         controlsContainer.querySelectorAll('.checklist-btn').forEach(btn => {
    ...
    function markdownToHtmlWithToc(markdown) { /* ... */
        ...
        let html = markdown
            .trim()
            // Headings (H1-H5) - Add IDs and checklist controls to H2 and H3
            .replace(/^##### (.*$)/gim, (match, content) => `<h5>${content}</h5>`)
            .replace(/^#### (.*$)/gim, (match, content) => `<h4>${content}</h4>`)
            .replace(/^### (.*$)/gim, (match, content) => {
                headingCounter++;
                const cleanContent = content.trim();
                const id = sanitizeForId(cleanContent) + '-' + headingCounter;
                headings.push({ id: id, text: cleanContent, level: 3 });
                // Wrap heading and controls in a div for layout
                return `<div class="heading-with-controls">
                            <h3 id="${id}">${cleanContent}</h3>
                            ${createChecklistControls(id)}
                        </div>`;
            })
            .replace(/^## (.*$)/gim, (match, content) => {
                 headingCounter++;
                 const cleanContent = content.trim();
                 const id = sanitizeForId(cleanContent) + '-' + headingCounter;
                 if (!cleanContent.toLowerCase().startsWith('appendix')) {
                    headings.push({ id: id, text: cleanContent, level: 2 });
                 }
                 // Wrap heading and controls in a div for layout
                 return `<div class="heading-with-controls">
                             <h2 id="${id}">${cleanContent}</h2>
                             ${createChecklistControls(id)}
                         </div>`;
            })
            .replace(/^# (.*$)/gim, (match, content) => `<h1 id="${sanitizeForId(content.trim())}">${content.trim()}</h1>`) // Main Title - no controls

            // Bold (**text**)
            .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
             // Italics (*text*) - Handle simple cases, avoid list markers
            .replace(/(?<!^[\s*]|\*)\*(.*?)\*(?![\s*]|\*)/g, '<em>$1</em>')
            // List items (- item) - Handle simple unordered lists
            .replace(/^\s*-\s+(.*$)/gim, '<li>$1</li>');

        // Wrap list items (as before)
        ...
        html = html.split(/\n\n+/)
            .map(paragraph => {
                ...
                if (!trimmed) return '';
                if (/^<(div|h[1-5]|ul|li|p|blockquote|pre)/i.test(trimmed)) { // Added div check
                    return trimmed;
                }
    ...
    function generateTocHtml(headings) { /* ... */
        let tocHtml = '';
        let currentArticleList = null; // To hold section items

        headings.forEach(heading => {
            if (heading.level === 2) { // Article (H2)
                // Close previous article's section list if open
                if (currentArticleList) {
                    tocHtml += currentArticleList + '</ul></li>';
                    currentArticleList = null;
                }
                tocHtml += `<li><a href="#${heading.id}">${heading.text}</a>`;
                // Prepare for potential sections under this article
                currentArticleList = '<ul>';
            } else if (heading.level === 3 && currentArticleList) { // Section (H3) under an Article
                currentArticleList += `<li><a href="#${heading.id}">${heading.text}</a></li>`;
            }
            // Ignore other levels for ToC for now
        });

        // Close the last article's section list if it exists
        if (currentArticleList) {
            tocHtml += currentArticleList + '</ul></li>';
        }
    ...
    function createSnippet(htmlContent, regex) { /* ... */
        ...
        const end = Math.min(textContent.length, matchIndex + match[0].length + snippetRadius);

        let snippetText = textContent.substring(start, end);

        if (start > 0 && textContent.charAt(start - 1) !== ' ') snippetText = '...' + snippetText;
        if (end < textContent.length && textContent.charAt(end) !== ' ') snippetText += '...';

        const highlightedSnippet = snippetText.replace(regex, (m) => `<span class="highlight">${m}</span>`);

        return highlightedSnippet;
     }

    // --- Core Application Logic ---
    function restoreFullView(targetId = null) { /* ... */
        constitutionContentEl.innerHTML = originalHtml; // Restore original content
        isShowingSearchResults = false;
        mainLayout.classList.remove('showing-results');

        // Apply stored checklist statuses
        const statuses = loadChecklistStatus();
        Object.keys(statuses).forEach(sectionId => {
            applyChecklistVisualStatus(sectionId, statuses[sectionId]);
        });

        // Scroll if targetId is provided
        if (targetId) {
            const targetElement = document.getElementById(targetId);
            if (targetElement) {
                setTimeout(() => {
                    // Find the container div to scroll into view
                    const container = targetElement.closest('.heading-with-controls') || targetElement;
                    container.scrollIntoView({ behavior: 'smooth', block: 'start' });
                     container.classList.add('highlight-scroll');
                     setTimeout(() => container.classList.remove('highlight-scroll'), 1500);
                }, 100);
            } else {
                console.warn(`Target element with ID "${targetId}" not found.`);
                constitutionContentEl.scrollTop = 0;
            }
        } else {
             constitutionContentEl.scrollTop = 0;
        }
    ...
    function performSearch() { /* ... */
        ...
        mainLayout.classList.add('showing-results');
        constitutionContentEl.innerHTML = ''; // Clear for results

        const fragment = document.createDocumentFragment();
        const resultsFound = [];

        const tempContainer = document.createElement('div');
        tempContainer.innerHTML = originalHtml; // Use original HTML (which includes IDs)

        const escapedSearchTerm = searchTerm.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
        const regex = new RegExp(escapedSearchTerm, 'gi');

        // Find headings with IDs (where checklists were added)
        const sectionHeadings = tempContainer.querySelectorAll('h2[id], h3[id]');

        sectionHeadings.forEach(heading => {
            let element = heading.closest('.heading-with-controls')?.nextElementSibling; // Start after the heading container
            let sectionContentHtml = ''; // Store HTML for snippet creation
            const sectionId = heading.id;
            const sectionTitle = heading.textContent;

            // Gather content until the next heading container
            while (element && !element.classList.contains('heading-with-controls')) {
                sectionContentHtml += element.outerHTML;
                element = element.nextElementSibling;
            }

             // Search within the text content of the gathered section
             const tempSectionDiv = document.createElement('div');
             tempSectionDiv.innerHTML = sectionContentHtml;
             const sectionText = tempSectionDiv.textContent || tempSectionDiv.innerText || '';

            if (sectionText.match(regex)) {
                const snippet = createSnippet(sectionContentHtml, regex); // Create snippet from section HTML
                resultsFound.push({
                    heading: { id: sectionId, text: sectionTitle },
                    snippet: snippet
                });
            }
        });
        ...
        resultsHeader.textContent = `Search Results for "${searchTerm}" (${resultsFound.length} found)`;
        resultsContainer.appendChild(resultsHeader);
        const backButton = document.createElement('button');
        backButton.textContent = '← Back to Full Constitution';
        backButton.className = 'back-to-full-view';
        backButton.onclick = () => restoreFullView();
        ...
        if (resultsFound.length > 0) {
            resultsFound.forEach(result => {
                ...
                resultItem.dataset.targetId = result.heading.id;
                const headingEl = document.createElement('h4');
                headingEl.textContent = result.heading.text;
        ...
        constitutionContentEl.appendChild(fragment);
        constitutionContentEl.scrollTop = 0;
     }
    function initializeApp() { /* ... */
        // 1. Convert Markdown, get headings, store original HTML
        const { html, headings } = markdownToHtmlWithToc(constitutionMarkdown);
        originalHtml = html;

        // 2. Display initial view (applies checklist statuses)
        restoreFullView();

        // 3. Generate and display ToC
        const tocHtml = generateTocHtml(headings);
        tocListEl.innerHTML = tocHtml;

        // 4. Add ToC link listeners (as before)
        tocListEl.querySelectorAll('a').forEach(anchor => {
            anchor.addEventListener('click', function (e) {
                e.preventDefault();
                const targetId = this.getAttribute('href').substring(1);
                if (isShowingSearchResults) {
                    restoreFullView(targetId);
                } else {
                    const targetElement = document.getElementById(targetId);
                    if (targetElement) {
                         // Find the container div to scroll into view
                        const container = targetElement.closest('.heading-with-controls') || targetElement;
                        container.scrollIntoView({ behavior: 'smooth', block: 'start' });
                        // Apply temporary highlight on scroll
                        container.classList.add('highlight-scroll');
                        setTimeout(() => container.classList.remove('highlight-scroll'), 1500);
                    }
                }
    ...
    // Listener for Clear Checklist button
    clearChecklistBtn.addEventListener('click', () => {
        if (confirm("Are you sure you want to clear all checklist statuses?")) {
            saveChecklistStatus({}); // Clear storage
            restoreFullView(); // Refresh the view to show cleared statuses
        }
    });

    // Click listener for search results AND checklist buttons (event delegation)
    constitutionContentEl.addEventListener('click', (event) => { /* ... */
        // Handle search result clicks
        const resultItem = event.target.closest('.search-result-item');
        if (resultItem && resultItem.dataset.targetId) {
            const targetId = resultItem.dataset.targetId;
            restoreFullView(targetId);
            return; // Stop processing if it was a search result click
        }

        // Handle checklist button clicks
        const checklistButton = event.target.closest('.checklist-btn');
        if (checklistButton) {
            const controlsContainer = checklistButton.closest('.checklist-controls');
            const sectionId = controlsContainer.dataset.sectionId;
            const newStatus = checklistButton.dataset.status;
            const currentStatus = loadChecklistStatus()[sectionId];

            if (currentStatus === newStatus) {
                // Clicked the already active button - clear the status
                updateSectionStatus(sectionId, null);
                applyChecklistVisualStatus(sectionId, null);
            } else {
                // Clicked a new status button
                updateSectionStatus(sectionId, newStatus);
                applyChecklistVisualStatus(sectionId, newStatus);
            }
        }
     });

    // Mobile ToC Toggle (as before)
    toggleTocButton.addEventListener('click', () => { /* ... */
        tocContainer.classList.toggle('visible');
     });
    document.addEventListener('click', (event) => { /* ... */
        if (window.innerWidth <= 768 && tocContainer.classList.contains('visible')) {
            if (!tocContainer.contains(event.target) && event.target !== toggleTocButton) {
                tocContainer.classList.remove('visible');
            }
        }
     });
